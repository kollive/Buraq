...
export class Purchase extends Component {
    ...
    if (this.props.cadetName != nextProps.cadetName) {
      ...
      /*
        ...
        this.props.getCadets({
            type: purchaseTypes.FETCH_TABLES_REQUEST,
            cname: nextProps.cadetName
          });
          */
    }

    if (nextProps.PurchaseState.items) {
      this.items = nextProps.PurchaseState.items;
    }
  ...
  constructor(props) {
    ...
    this.state = {
      ...
      filterValue: "",
      sortAsc: true,
      sortedCol: "hv_po_vendor_name",
      searchCol: "hv_po_vendor_name",
      pageSize: 10,
      dropdownOpen: false,
      popoverOpen: false,
      inputSearch: "",
  ...
  clickedItem(item, e) {
    return;
    debugger;
    this.filterValue = item[this.state.searchCol].toLowerCase();
    this.setState({
      popoverOpen: false
    });
  ...
  _onFilterChange() {
    ...
    const size = this.props.PurchaseState.items.length;

    let filteredItems = [];

    for (var index = 0; index < size; index++) {
      const { hv_po_vendor_name } = this.props.PurchaseState.items[index];

      if (hv_po_vendor_name.toLowerCase().indexOf(filterBy) !== -1) {
        filteredItems.push(this.props.PurchaseState.items[index]);
      }
    ...
    this.props.makeRowEditable({
      type: purchaseTypes.MAKE_ROW_EDITABLE,
      payload: {
        rowID: -1
      }
    });
              ...
              <Row className="p-0 m-0">
                ...
                <Col sm="4">
                  <div className="mx-center">
                    {this.state.pageOfItems.length} Vendors
                  </div>
                </Col>
                <Col sm="3">
                  ...
                  >
                    <thead>
                      <tr style={{ backgroundColor: "grey", color: "white" }}>
                        ...
                        <th
                          style={styles.link}
                          onClick={() => this.sortTable("hv_po_num")}
                        >
                          PO Number {" "}
                          <i
                        ...
                        <th
                          style={styles.link}
                          onClick={() => this.sortTable("hv_po_vendor_id")}
                        >
                          Vendor ID {" "}
                          <i
                            className={this.RenderHeaderColumn(
                        ...
                        <th
                          style={styles.link}
                          onClick={() => this.sortTable("hv_po_vendor_name")}
                        >
                          Vendor Name{" "}
                          <i
                            className={this.RenderHeaderColumn(
                        ...
                        <th
                          style={styles.link}
                          onClick={() => this.sortTable("hv_po_item_id")}
                        >
                          Item ID{" "}
                          <i
                            className={this.RenderHeaderColumn(
                        ...
                        <th
                          style={styles.link}
                          onClick={() => this.sortTable("hv_po_occ")}
                        >
                          OCC{" "}
                          <i
                            className={this.RenderHeaderColumn(
                        ...
                        <th
                          style={styles.link}
                          onClick={() => this.sortTable("hv_po_status")}
                        >
                          Status{" "}
                          <i
                            className={this.RenderHeaderColumn(
                    ...
                    <tbody>
                      {this.state.pageOfItems.map((row, index) => (
                        <tr key={index} onClick={() => this.showDetails(row)}>
                          ...
                          <td>{row.hv_po_num}</td>
                          <td>{row.hv_po_vendor_id}</td>
                          <td>{row.hv_po_vendor_name}</td>
                          <td>{row.hv_po_item_id}</td>
                          <td>{row.hv_po_occ}</td>
                          <td>{row.hv_po_status}</td>
                          <td>{row.hv_po_amt}</td>
                        </tr>
                      ))}
                      <tr
