...
import { actions as budgetActions } from "reducers/budgetreducer";
...
export class Budget extends Component {
    ...
    if (this.props.cadetName != nextProps.cadetName) {
      ...
      /*
        ...
        this.props.getCadets({
            type: budgetTypes.FETCH_TABLES_REQUEST,
            cname: nextProps.cadetName
          });
          */
    }

    if (nextProps.BudgetState.items) {
      this.items = nextProps.BudgetState.items;
    }
  ...
  _onFilterChange() {
    ...
    const size = this.props.BudgetState.items.length;

    let filteredItems = [];

    for (var index = 0; index < size; index++) {
      const { hv_bdgt_res_name } = this.props.BudgetState.items[index];

      if (hv_bdgt_res_name.toLowerCase().indexOf(filterBy) !== -1) {
        filteredItems.push(this.props.BudgetState.items[index]);
      }
    ...
    this.props.makeRowEditable({
      type: budgetTypes.MAKE_ROW_EDITABLE,
      payload: {
        rowID: -1
      }
    });
                  ...
                  >
                    <thead>
                      <tr style={{ backgroundColor: "grey", color: "white" }}>
                        ...
                        <th
                          style={styles.link}
                          onClick={() => this.sortTable("hv_bdgt_cat")}
                        >
                          Budget Category {" "}
                          <i
                        ...
                        <th
                          style={styles.link}
                          onClick={() => this.sortTable("hv_bdgt_res_name")}
                        >
                          Staff / Resource Name {" "}
                          <i
                        ...
                        <th
                          style={styles.link}
                          onClick={() => this.sortTable("hv_bdgt_title")}
                        >
                          Title{" "}
                          <i
                    ...
                    <tbody>
                      {this.state.pageOfItems.map((row, index) => (
                        <tr key={index} onClick={() => this.showDetails(row)}>
                          ...
                          <td>{row.hv_bdgt_cat}</td>
                          <td>{row.hv_bdgt_res_name}</td>
                          <td>{row.hv_bdgt_title}</td>
                          <td>{row.hv_bdgt_obj_code}</td>
                          <td>{row.hv_bdgt_amt}</td>
                          <td>{row.hv_commit_amt}</td>
                        </tr>
                      ))}
                      <tr
